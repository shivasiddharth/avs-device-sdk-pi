...
SampleAppReturnCode UserInputManager::run() {
    ...
    while (true) {
        ...
        char socmsg;
        socmsg = ::tolower(buffer)
        char x;
        if (!readConsoleInput(&x)) {
            break;
        }
        x = ::tolower(x);
        if (x == QUIT || socmsg == QUIT) {
            break;
        } else if (x == RESET || socmsg == RESET) {
            if (confirmReset()) {
                userTriggeredLogout = true;
            }
        } else if (x == REAUTHORIZE || socmsg == REAUTHORIZE) {
            confirmReauthorizeDevice();
        } else if (x == MIC_TOGGLE || socmsg == MIC_TOGGLE) {
            m_interactionManager->microphoneToggle();
        } else if (x == STOP || socmsg == STOP) {
            m_interactionManager->stopForegroundActivity();
        } else if (x == SPEAKER_CONTROL || socmsg == SPEAKER_CONTROL) {
            controlSpeaker();
#ifdef ENABLE_PCC
} else if (x == PHONE_CONTROL || socmsg == PHONE_CONTROL) {
            controlPhone();
#endif
#ifdef ENABLE_MCC
} else if (x == MEETING_CONTROL || socmsg == MEETING_CONTROL) {
            controlMeeting();
#endif
} else if (x == SETTINGS || socmsg == SETTINGS) {
            settingsMenu();
#ifdef ENABLE_ENDPOINT_CONTROLLERS
} else if (x == ENDPOINT_CONTROLLER || socmsg == ENDPOINT_CONTROLLER) {
            endpointControllerMenu();
        } else if (x == DYNAMIC_ENDPOINT_SCREEN || socmsg == DYNAMIC_ENDPOINT_SCREEN) {
            dynamicEndpointModificationMenu();
#endif
} else if (x == INFO || socmsg == INFO) {
        ...
        } else if (x == HOLD || socmsg == HOLD) {
            m_interactionManager->holdToggled();
        } else if (x == TAP || socmsg == TAP) {
            m_interactionManager->tap();
        } else if (x == PLAY || socmsg == PLAY) {
            m_interactionManager->playbackPlay();
        } else if (x == PAUSE || socmsg == PAUSE) {
            m_interactionManager->playbackPause();
        } else if (x == NEXT || socmsg == NEXT) {
            m_interactionManager->playbackNext();
        } else if (x == PREVIOUS || socmsg == PREVIOUS) {
            m_interactionManager->playbackPrevious();
        } else if (x == SKIP_FORWARD || socmsg == SKIP_FORWARD) {
            m_interactionManager->playbackSkipForward();
        } else if (x == SKIP_BACKWARD || socmsg == SKIP_BACKWARD) {
            m_interactionManager->playbackSkipBackward();
        } else if (x == SHUFFLE || socmsg == SHUFFLE) {
            m_interactionManager->playbackShuffle();
        } else if (x == LOOP || socmsg == LOOP) {
            m_interactionManager->playbackLoop();
        } else if (x == REPEAT || socmsg == REPEAT) {
            m_interactionManager->playbackRepeat();
        } else if (x == THUMBS_UP || socmsg == THUMBS_UP) {
            m_interactionManager->playbackThumbsUp();
        } else if (x == THUMBS_DOWN || socmsg == THUMBS_DOWN) {
            m_interactionManager->playbackThumbsDown();
        } else if (x == FIRMWARE_VERSION || socmsg == FIRMWARE_VERSION) {
            std::string text;
...
#ifdef ENABLE_COMMS
} else if (x == COMMS_CONTROL || socmsg == COMMS_CONTROL) {
...
#endif
#ifdef DIAGNOSTICS
} else if (x == DIAGNOSTICS_CONTROL || socmsg == DIAGNOSTICS_CONTROL) {
            diagnosticsMenu();
#endif
} else if (x == DEVICE_SETUP_COMPLETE || socmsg == DEVICE_SETUP_COMPLETE) {
